<?php
  ...
  class Account_detail_model extends CI_Model {
    ...
    // get city
  public function get_user_detail($type, $username)
    {
       switch ($type) {
        case "farmer":
       ...
       if($type == 'farmer' || $type == 'fpo' || $type == 'trader' || $type == 'processor' || $type == 'buyer'){ 
     $this->db->select('*');  
     $this->db->join($table_name, $pr_table.'.id = '.$table_name.'.id');
     $this->db->where($table_name.'.username', $username);
     $querys = $this->db->get($pr_table);
      // return $result=$querys->row();
