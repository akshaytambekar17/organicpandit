...
class org_input_model extends CI_Model {

	
	public function insert_org_input($profile, $company_images, $video, $catalouge){
// User data array
		print "in";
		$data = array(
...
// Insert user
		$this->db->insert('tbl_org_input', $data);
		$insert_id = $this->db->insert_id();
					$data2 = array(
			'user_id' => $insert_id,
			'user_type' => "organic input"
		);
		...
		$this->db->insert('tbl_notify', $data3);
				...
				if($insert_id)
			{
				$row=$this->db->query("select * from `tbl_org_input` where `id`='$insert_id'")->row_array();
				$to = $row['email'];
