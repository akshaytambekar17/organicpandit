...
class Exhibitor_model extends CI_Model {
	...
	public function insert_exhibitor($profile, $company_images, $video){
// User data array
		print "in";
		$data = array(
...
// Insert user
		$this->db->insert('tbl_exhibitors', $data);
		...
		$this->db->insert('tbl_notify', $data3);
				...
				if($insert_id)
			{
				$row=$this->db->query("select * from `tbl_exihibitors` where `id`='$insert_id'")->row_array();
				$to = $row['email'];
