...
class Trader_model extends CI_Model {
	...
	public function insert_trader($profile, $certify_img, $video){
// User data array
		print "in";
...
// Insert user
		$this->db->insert('tbl_trader', $data);
	...
	//	}
		...
		if($insert_id)
			{
				$row=$this->db->query("select * from `tbl_trader` where `id`='$insert_id'")->row_array();
				$to = $row['email'];
...
// file upload in product  folder end   
		...
		for ($i=0; $i < $item_count; $i++) {
			...
			$this->db->insert("tbl_pr_trader",$pr_data);
	...
	{
		$this->db->select('*')->from('tbl_pr_trader');
		$query=$this->db->get();
		return $query->result();
	}
