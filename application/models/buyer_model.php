...
class Buyer_model extends CI_Model {
	...
	public function insert_buyer($profile, $certify_img, $video){
// User data array
		print "in";
...
// Insert user
		...
		if($insert_id)
			{
				$row=$this->db->query("select * from `tbl_buyer` where `id`='$insert_id'")->row_array();
				$to = $row['email'];
...
// file upload in product  folder end   
		...
		for ($i=0; $i < $item_count; $i++) {
			...
			$this->db->insert("tbl_pr_buyer",$pr_data);
...
public function get_products()
	{
		$this->db->select('*')->from('tbl_pr_buyer');
		$query=$this->db->get();
		return $query->result();
	}
	// get city
