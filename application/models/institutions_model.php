...
class Institutions_model extends CI_Model {
	...
	public function insert_institutions($profile, $company_images, $video, $catalouge){
// User data array
		print "in";
		$data = array(
...
// Insert user
		$this->db->insert('tbl_institutions', $data);
		...
		$this->db->insert('tbl_notify', $data3);
				...
				if($insert_id)
			{
				$row=$this->db->query("select * from `tbl_institutions` where `id`='$insert_id'")->row_array();
				$to = $row['email'];
