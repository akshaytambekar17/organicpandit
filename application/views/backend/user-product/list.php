                ...
                <div class="box">
                    <div class="box-header">
                        <div class="pull-right">
                            <a href="<?= base_url()?>admin/user/user-products/add" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i> Add Product</a>
                        </div>
                    </div>
                  <!-- /.box-header -->
                    <div class="box-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover js-datatable-list">
                                ...
                                <tbody>
                                <?php
                                    if( true == isArrVal( $arrUserProductsList ) ) {
                                        foreach( $arrUserProductsList as $arrUserProductDetails ) {
                                ?>
                                            <tr class="gradeX" id="order-<?= $arrUserProductDetails['id'] ?>">
                                                <td class="hidden"><?= $arrUserProductDetails['id']; ?></td>
                                                <td><?= $arrUserProductDetails['fullname']; ?></td>
                                                <td><?= $arrUserProductDetails['user_type_name']; ?></td>
                                                <td><?= $arrUserProductDetails['name']; ?></td>
                                                <td><?php
                                                        $arrQuantites = getQuantities();
                                                        echo $arrQuantites[$arrUserProductDetails['quantity_id']]; ?>
                                                </td>
                                                <td><?= $arrUserProductDetails['price']; ?></td>
                                                <td>
                                                    <a href="<?= base_url()?>admin/user/user-products/update?id=<?= $arrUserProductDetails['id']?>"  name="update-product" data-toggle="tooltip" title="Update"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                                    <a href="javascript:void(0)" data-user_product_id="<?= $arrUserProductDetails['id'] ?>" data-product_name="<?= $arrUserProductDetails['name'] ?>" name="delete-product" data-toggle="tooltip" title="Delete" onclick="showDeleteModal(this)"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                                </td>
                                            </tr>
                                <?php
                                        }
        ...
        <div class="modal-content">
            <div class="modal-body">
                ...
                <div class="text-center popup-content">  
                    ...
                    <input  type="hidden" id="js-modal-user-product-id"> 
...
<script>
    $(document).ready(function () {
        $("#js-confirm-button").on('click',function() {
            var intUserProductId = $("#js-modal-user-product-id").val();
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>" + "admin/user/user-products/delete",
                data: { 'id' : intUserProductId },
    ...
    function showDeleteModal(ths){
        var intUserProductId = $(ths).data('user_product_id');
        $("#js-modal-content-name").text( $(ths).data('product_name') );
        $("#js-modal-user-product-id").val(intUserProductId);
