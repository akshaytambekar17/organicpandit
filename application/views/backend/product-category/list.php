                ...
                <div class="box">
                    <div class="box-header">
                        <div class="pull-right">
                            <a href="<?= base_url()?>admin/product-category/add" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i> Add Category</a>
                        </div>
                    </div>
                  <!-- /.box-header -->
                    <div class="box-body">
                            ...
                            <table class="table table-striped table-bordered table-hover datatable-list">
                                ...
                                ?>
                                            <tr class="gradeX" id="order-<?= $value['id'] ?>">
                                                ...
                                                <td>
                                                    <a href="<?= site_url('admin/product-category/update?id='.$value['id'])?>" class="btn btn-primary view-product" data-id="<?= $value['id'] ?>" name="view-product">Edit</a>
                                                    <a href="javascript:void(0)" class="btn btn-danger delete-product" data-id="<?= $value['id'] ?>" name="delete-product" onclick="productCategoryDelete(this)">Delete</a><br>
                                                </td>
                                            </tr>
                                            <?php
                                        }
...
<script>
    $(document).ready(function () {
        $("#js-confirm-button").on('click',function(){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>" + "admin/product-category/delete",
                data: { 'id' : id },
    ...
    function productCategoryDelete(ths){
        var id = $(ths).data('id');
        $("#id_modal").val(id);
