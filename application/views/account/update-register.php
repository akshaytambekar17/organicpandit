...
foreach ($tdata as $key => $row){
	...
	<form   method="POST" role="form" id="farmerRegisterForm" enctype="multipart/form-data" accept-charset="utf-8">
		<div class="row">
			<?php if($user_type=="farmer" || $user_type=="fpo" || $user_type=="trader" || $user_type=="processor" || $user_type=="buyer" || $user_type=="org_consultant" || $user_type=="org_input" || $user_type=="packaging" ||$user_type=="logistic" || $user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant" || $user_type=="ngo"){?>
			...
			<?php } ?>      
			<?php if($user_type=="fpo" || $user_type=="trader" || $user_type=="processor" || $user_type=="buyer" || $user_type=="org_input" || $user_type=="packaging" || $user_type=="logistic" ||$user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant" ){?>
			<div class="col-md-4 form-group">
			...
			<?php } ?>
			<?php if($user_type=="org_input" || $user_type=="packaging" || $user_type=="logistic" ||$user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant"){?>
			<div class="col-md-4 form-group">
			...
			<?php } ?>
			<?php if($user_type=="fpo" || $user_type=="trader" || $user_type=="processor" || $user_type=="buyer" || $user_type=="org_consultant" || $user_type=="org_input" || $user_type=="packaging" ||$user_type=="logistic" || $user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant" || $user_type=="ngo"){?>
			<div class="col-md-4 form-group">
			...
			<?php } ?>
			<?php if($user_type=="farmer" || $user_type=="fpo" || $user_type=="trader" || $user_type=="processor" || $user_type=="buyer"){?>
			<div class="col-md-4 form-group">
			...
			<?php } ?>
			<?php if($user_type=="farmer" || $user_type=="fpo" || $user_type=="trader" || $user_type=="processor"){?>
			<div class="col-md-4 form-group">
			...
			<?php if($user_type=="buyer" || $user_type=="org_consultant" || $user_type=="org_input" || $user_type=="packaging" ||$user_type=="logistic" || $user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant" || $user_type=="ngo"){?>
			<div class="col-md-4 form-group">
		...
		<div class="row">
			<?php if($user_type=="farmer" || $user_type=="fpo" || $user_type=="trader" || $user_type=="processor" || $user_type=="buyer" || $user_type=="org_consultant" || $user_type=="org_input" || $user_type=="packaging" ||$user_type=="logistic" || $user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant" || $user_type=="ngo"){?>
			<div class="col-md-4 form-group">
				<label class="control-label requiredlbl" for="profile"> Profile Image:</label>
				<div style="display:none;" class="profileFile">
					<input type="file" class="form-control" id="profile"  name="profile">
					<input type="text" class="form-control hidden" id="old_profile"  name="old_profile" value="<? echo $row->profile; ?>">
					<a href="javascript:void(0);" class="btn btn-primary btn-sm chngBtn" id="changeImg" onclick="dontChngProfileFun()" > Don't Change</a>
				</div>
				<div class="profile-box">
					<?php if(!empty($row->profile)){ ?>
					<img src="<?php echo base_url(); ?>upload/profile/<?php echo $row->profile; ?>" class="img-responsive chngImg">
					<?
				}else{
					echo '<p>certificate is not available</p>';
				} ?>
					<a href="javascript:void(0);" class="btn btn-primary btn-sm chngBtn" id="changeImg" onclick="chngProfileFun()" > Change Image</a>
				</div>
			</div>
			<?php } ?>
			<?php if($user_type=="farmer" || $user_type=="fpo" || $user_type=="trader" || $user_type=="processor"){?>
			<div class="col-md-4 form-group">
				<label class="control-label requiredlbl" for="certify_img"> Certification Image:</label>
				<div style="display:none;" class="certifyFile">
					<input type="file" class="form-control" id="certify_img"  name="certify_img">
					<input type="text" class="form-control hidden" id="old_certify_img"  name="old_certify_img" value="<? echo $row->certify_img; ?>">
					<a href="javascript:void(0);" class="btn btn-primary btn-sm chngBtn" id="changeImg" onclick="dontChngCertifyFun()" > Don't Change</a>
				</div>
				<div class="certify-box">
					<?php if(!empty($row->certify_img)){ ?>
					<img src="<?php echo base_url(); ?>upload/certificate/<?php echo $row->certify_img; ?>" class="img-responsive chngImg">
					<?
				}else{
					echo '<p>certificate is not available</p>';
				} ?>
					<a href="javascript:void(0);" class="btn btn-primary btn-sm chngBtn" id="changeImg" onclick="chngCertifyFun()" > Change Image</a>
				</div>
			</div>
			<?php } ?>
			<?php if($user_type=="farmer" || $user_type=="fpo" || $user_type=="trader" || $user_type=="processor" || $user_type=="buyer" || $user_type=="org_consultant" || $user_type=="org_input" || $user_type=="packaging" ||$user_type=="logistic" || $user_type=="farm_equipment" || $user_type=="exhibitors" || $user_type=="shops" || $user_type=="labs" || $user_type=="gov_agency" || $user_type=="institutions" || $user_type=="others" || $user_type=="restaurant" || $user_type=="ngo"){?>
			<div class="col-md-4 form-group">
				<label class="control-label requiredlbl" for="video"> video :</label>
				<div style="display:none;" class="videoFile">
					<input type="file" class="form-control" id="video"  name="video" accept="video/*">
					<input type="hidden" class="form-control" id="old_video"  name="old_video" value="<? echo $row->video; ?>">

					<a href="javascript:void(0);" class="btn btn-primary btn-sm chngBtn" id="changeImg" onclick="dontChngVideoFun()" > Don't Change</a>
				</div>
				<div class="video-box">
				<?php if(!empty($row->video)){ ?>
				<video class="chngImg" controls >
						<source src="<?php echo base_url(); ?>upload/video/<?php echo $row->video; ?>" type="video/mp4">
							Your browser does not support the video tag.
						</video>
				<?
				}else{
					echo '<p>Video is not available</p>';
				} ?>	
						<a href="javascript:void(0);" class="btn btn-primary btn-sm chngBtn" id="changeImg" onclick="chngVideoFun()" > Change Video</a>
					</div>
				</div>
				<?php } ?>
			</div>
	...
	<script type="text/javascript">
		...
		function chngProfileFun() {
			$('.profileFile').show();
			$('.profile-box').hide();
		}
		function dontChngProfileFun() {
			$('.profileFile').hide();
			$('.profile-box').show();
		}
		function chngCertifyFun() {
			$('.certifyFile').show();
			$('.certify-box').hide();
		}
		function dontChngCertifyFun() {
			$('.certifyFile').hide();
			$('.certify-box').show();
		}
		function chngVideoFun() {
			$('.videoFile').show();
			$('.video-box').hide();
		}
		function dontChngVideoFun() {
			$('.videoFile').hide();
			$('.video-box').show();
		}
