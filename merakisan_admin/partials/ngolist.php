					...
					<?php
						...
						{	
							...
							$id= $_GET['id'];
							$sql = "select * from `tbl_ngo` where id='$id'";
						...
						{
							$did= $_GET['did'];
							$sql = "delete  from `tbl_ngo` where `id`='$did'";
				...
				$query = mysqli_query($db, "INSERT INTO `tbl_ngo`(`fullname`, `username`,`email`,`landline`,`mobile`,`state`,`city`,`address`,`gst`,
							...
							if (!$query) {	
								...
								if($query)
								{
								 $msg = "<div class='alert alert-block alert-success'>
										<button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button>
										<i class='ace-icon fa fa-check green'></i> &nbsp; Thank You! NGO Add successfully.</div>"; 
								}
							}
						} 
						...
						{	
						   ...
						   $rand2 = rand(100000,999999);
					        ...
					        $query = mysqli_query($db, "UPDATE `tbl_ngo` SET `fullname`='$fullname', `username`='$username',`email`='$email',`landline`='$landline',`mobile`='$mobile',`state`='$state',`city`='$city',`address`='$address',`gst`='$gst',
					        ...
					        $query = mysqli_query($db, "UPDATE `tbl_ngo` SET `fullname`='$fullname', `username`='$username',`email`='$email',`landline`='$landline',`mobile`='$mobile',`state`='$state',`city`='$city',`address`='$address',`gst`='$gst',
							...
							else
							  {
								...
								if($query)
								{
								 $msg = "<div class='alert alert-block alert-success'>
										<button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button>
										<i class='ace-icon fa fa-check green'></i> &nbsp; Thank You! NGO Updated successfully.</div>"; 
								}
						}
							...
							<div class="col-xs-12 datatable_wrapper">
								...
								<table id="myTable" class="display nowrap table  table-striped table-responsive" style="overflow-x:auto">  
									...
									?>
										...
										<td><a href="addngo.php?id=<?php echo $row['id'];?>"><i class="fa fa-pencil-square-o fa-2x" aria-hidden="true" style="color:#007700"></i></a> </td>
										<td><a href="addngo.php?did=<?php echo $row['id'];?>" onclick="return confirm('<?php echo "Do you want to delete?";?>')"><i class="fa fa-trash-o fa-2x" aria-hidden="true" style="color:#007700"></i></a> </td>
															</tr> 
									<?php $no++;
									} ?>
									</tbody>  
