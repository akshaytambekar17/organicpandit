...
include ("includes/db.php");
			...
			<div class="col-md-1"> </div>
				<div class="col-md-12"> 
					<div class="panel"style="margin-bottom: 0; margin-top: 13px;">
						...
						<h5 align="center"><b>Today Order</b></h5><br>
							<div class="row">
							<?php 
  ...
  DISTINCT(o.`order_id`) ,SUM(f.`offer_price`*f.`product_qty`) AS cash ,c.`city_name`,o.`city_id`,o.`shipping_address`,o.`mobileno`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status`,o.`custname`,o.`count`FROM`orderdetail`o JOIN `city` c ON o.`city_id` = c.`id` JOIN `offer_order` f ON o.`order_id` = f.`order_id` WHERE f.`date`>= '$todate' GROUP BY o.`order_id` DESC"/*"SELECT  c.`city_name`,o.`city_id`,o.`order_id`,o.`shipping_address`,o.`mobileno`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status`,o.`custname`,o.`count`,f.`offer_price` FROM `orderdetail` o JOIN `city` c ON o.`city_id` = c.`id` JOIN `offer_order` f ON o.`order_id` = f.`order_id` WHERE o.`order_status`=0 order by o.`order_id` DESC"*/);
			...
			<br>
			<div class="row">
		 <center><button type="submit" onclick="myFunction('printdiv')" class="btn btn-sm btn-primary">Print</button>&nbsp<a href="export12.php"><button class="btn btn-sm  btn-success">Export</button></a></center>	 
		 </div>
