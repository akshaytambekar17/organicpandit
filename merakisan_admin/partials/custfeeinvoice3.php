			...
			<div class="col-md-1"> </div>
				<div class="col-md-12"> 
					<div class="panel"style="margin-bottom: 0; margin-top: 13px;">
						...
						<h5 align="center"><b>Complete Order</b></h5><br>
							<div class="row">
							<?php 
								if(empty($cityid))
								{
							$rs_result = mysqli_query($db,"SELECT 
  DISTINCT(o.`order_id`) ,SUM(f.`offer_price`) AS cash ,c.`city_name`,o.`city_id`,o.`shipping_address`,o.`mobileno`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status`,o.`custname`,o.`count`FROM`orderdetail`o JOIN `city` c ON o.`city_id` = c.`id` JOIN `offer_order` f ON o.`order_id` = f.`order_id` WHERE o.`order_status`=1 GROUP BY o.`order_id` DESC");
						}else{
						    $rs_result = mysqli_query($db,"SELECT 
  DISTINCT(o.`order_id`) ,SUM(f.`offer_price`) AS cash ,c.`city_name`,o.`city_id`,o.`shipping_address`,o.`mobileno`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status`,o.`custname`,o.`count`FROM`orderdetail`o JOIN `city` c ON o.`city_id` = c.`id` JOIN `offer_order` f ON o.`order_id` = f.`order_id` WHERE o.`city_id` = '$cityid' AND o.`order_status`=1 GROUP BY o.`order_id` DESC");	
						}
							?>
								<div class="col-md-12">
									 <table class="table">
			...
			<br>
			<div class="row">
		 <center><button type="submit" onclick="myFunction('printdiv')" class="btn btn-sm btn-primary">Print</button>&nbsp<a href="export7.php"><button class="btn btn-sm  btn-success">Export</button></a></center>	 
		 </div>
