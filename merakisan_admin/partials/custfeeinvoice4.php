...
include ("includes/db.php");
		...
		<div class="container" style="background:#333333">
			...
			<div class="col-md-1"> </div>
				<div class="col-md-12"> 
					<div class="panel"style="margin-bottom: 0; margin-top: 13px;">
						...
						<h5 align="center"><b>Processing Order</b></h5><br>
							<div class="row">
							<?php 
							 
								if(empty($cityid))
								{
							$rs_result = mysqli_query($db,"SELECT d.`name`,d.`mobile`,o.`order_id`,o.`shipping_address`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status` FROM `assign_boy` a JOIN `delivery_boy` d ON a.`boyid`= d.`id` JOIN `orderdetail` o ON a.`order_id` = o.`order_id` WHERE o.`order_status`=0 GROUP BY o.`order_id` DESC ");
						}else{
							$rs_result = mysqli_query($db,"SELECT d.`name`,d.`mobile`,o.`order_id`,o.`shipping_address`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status` FROM `assign_boy` a JOIN `delivery_boy` d ON a.`boyid`= d.`id` JOIN `orderdetail` o ON a.`order_id` = o.`order_id` WHERE o.`city_id` = '$cityid' AND o.`order_status`=0 GROUP BY o.`order_id` DESC ");
						     }
							?>
								<div class="col-md-12">
									 <table class="table">
			...
			<br>
			<div class="row">
		 <center><button type="submit" onclick="myFunction('printdiv')" class="btn btn-sm btn-primary">Print</button>&nbsp<a href="export8.php"><button class="btn btn-sm  btn-success">Export</button></a></center>	 
		 </div>
