...
include ("includes/db.php");
		...
		<div class="container" style="background:#333333">
			...
			<div class="col-md-1"> </div>
				<div class="col-md-12"> 
					<div class="panel"style="margin-bottom: 0; margin-top: 13px;">
						...
						<h5 align="center"><b>Today Products</b></h5><br>
							<div class="row">
							<?php 
							 $date = new DateTime($date_ymd);
                                                          $dateonly=$date->format('Y-m-d');
			                                  $todate=$dateonly;
							$rs_result = mysqli_query($db,"SELECT  SUM(f.`product_qty`) AS Qty, f.`order_id`,f.`product_qty`,f.`product_name`,f.`offer_price`,f.`date`,o.`price_unit`,o.`category` FROM `offer_order`f,`offer_productlist`o WHERE o.`product_id` = f.`product_id` AND f.`date`>= '$todate' group by f.`product_id` "/*"SELECT  c.`city_name`,o.`city_id`,o.`order_id`,o.`shipping_address`,o.`mobileno`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status`,o.`custname`,o.`count`,f.`offer_price` FROM `orderdetail` o JOIN `city` c ON o.`city_id` = c.`id` JOIN `offer_order` f ON o.`order_id` = f.`order_id` WHERE o.`order_status`=0 order by o.`order_id` DESC"*/);
							?>
								<div class="col-md-12">
									 <table class="table">
									 <thead>
							...
							<?php   
									...
									?>
									 ...
									 <td> Rs.<?php echo $row['offer_price']; ?></td>
									 <td> <?php echo $row['category'];?>-<?php echo $row['price_unit']; ?></td>
									 <?php $subtotal = $row['Qty']*$row['offer_price']; ?>
									...
									<td colspan="5" align="center"><b>Sub Total Price:&nbsp;Rs.&nbsp;</b></td>
			...
			<br>
			<div class="row">
		 <center><button type="submit" onclick="myFunction('printdiv')" class="btn btn-sm btn-primary">Print</button>&nbsp<a href="export11.php"><button class="btn btn-sm  btn-success">Export</button></a></center>	 
		 </div>
