					...
					<?php
						...
						{	
						    ...
						    $id= $_GET['id'];
							$sql = "select * from `tbl_pr_processor` where id='$id'";
						...
						{
							$did= $_GET['did'];
							$sql = "delete  from `tbl_pr_processor` where `id`='$did'";
				...
				$query = mysqli_query($db, "INSERT INTO `tbl_pr_processor`(`pr_name`,`pr_desc`,`pr_avlFrom`,`pr_avlTo`,`pr_qty`,`pr_quality`,`pr_price`,`pr_image`) VALUES 
							...
							if (!$query) {	
								...
								if($query)
								{
								 $msg = "<div class='alert alert-block alert-success'>
										<button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button>
										<i class='ace-icon fa fa-check green'></i> &nbsp; Thank You! Product Processor Add successfully.</div>"; 
								}
							}
						}
						...
						{	
						   ...
						   $rand = rand(100000,999999); 
					        ...
					        $query = mysqli_query($db, "UPDATE `tbl_pr_processor` SET `pr_name`='$name',`pr_desc`='$discription',
							  	...
							  	else
							  {
								...
								if($query)
								{
								 $msg = "<div class='alert alert-block alert-success'>
										<button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button>
										<i class='ace-icon fa fa-check green'></i> &nbsp; Thank You! Product Processor Updated successfully.</div>"; 
								}
						}
					...
					<div class="page-content">
						...
						<div class="row <?php if(isset($_GET['id'])){} else {echo "hideshow"; } ?>" id="addprprocessor">
							<div class="col-xs-12">
	<!---------------------------PAGE CONTENT BEGINS------------------------------>
						...
						<div class="row">
							<div class="col-xs-12 dataTables_wrapper">
								...
								<?php
									...
									$sql = "select * from `tbl_pr_processor`";
								...
								<table id="myTable" class="display nowrap table  table-striped table-responsive" style="overflow-x:auto">  
									...
									<td><?php echo $no; ?></td> 
										...
										<td><a href="addprprocessor.php?id=<?php echo $row['id'];?>"><i class="fa fa-pencil-square-o fa-2x" aria-hidden="true" style="color:#007700"></i></a> </td>
										<td><a href="addprprocessor.php?did=<?php echo $row['id'];?>" onclick="return confirm('<?php echo "Do you want to delete?";?>')"><i class="fa fa-trash-o fa-2x" aria-hidden="true" style="color:#007700"></i></a> </td>															</tr> 
									<?php $no++;
									} ?>
									</tbody>  
								</table>  
