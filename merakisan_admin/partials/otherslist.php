					...
					<?php
						...
						{	
							...
							$id= $_GET['id'];
							$sql = "select * from `tbl_others` where id='$id'";
						...
						{
							$did= $_GET['did'];
							$sql = "delete  from `tbl_others` where `id`='$did'";
				...
				$query = mysqli_query($db, "INSERT INTO `tbl_others`(`fullname`, `username`,`email`,`landline`,`mobile`,`state`,`city`,`address`,`gst`,
							...
							if (!$query) {	
								...
								if($query)
								{
								 $msg = "<div class='alert alert-block alert-success'>
										<button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button>
										<i class='ace-icon fa fa-check green'></i> &nbsp; Thank You! Others Add successfully.</div>"; 
								}
							}
						} 
						...
						{	
						   ...
						   $rand3 = rand(100000,999999);
					        ...
					        $query = mysqli_query($db, "UPDATE `tbl_others` SET `fullname`='$fullname', `username`='$username',`email`='$email',`landline`='$landline',`mobile`='$mobile',`state`='$state',`city`='$city',`address`='$address',`gst`='$gst',
					        ...
					        $query = mysqli_query($db, "UPDATE `tbl_others` SET `fullname`='$fullname', `username`='$username',`email`='$email',`landline`='$landline',`mobile`='$mobile',`state`='$state',`city`='$city',`address`='$address',`gst`='$gst',
							...
							else
							  {
								...
								if($query)
								{
								 $msg = "<div class='alert alert-block alert-success'>
										<button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button>
										<i class='ace-icon fa fa-check green'></i> &nbsp; Thank You! Others Updated successfully.</div>"; 
								}
						}
					...
					<?php $user_check; //echo $date_ymd; //Display User Name ?> 
						...
						<div class="row <?php if(isset($_GET['id'])){} else {echo "hideshow"; } ?>" id="addothers">
							<div class="col-xs-12">
	<!---------------------------PAGE CONTENT BEGINS------------------------------>
							...
							<div class="col-xs-12 datatable_wrapper">
								...
								<table id="myTable" class="display nowrap table  table-striped table-responsive" style="overflow-x:auto">  
									...
									?>
										...
										<td><a href="addothers.php?id=<?php echo $row['id'];?>"><i class="fa fa-pencil-square-o fa-2x" aria-hidden="true" style="color:#007700"></i></a> </td>
										<td><a href="addothers.php?did=<?php echo $row['id'];?>" onclick="return confirm('<?php echo "Do you want to delete?";?>')"><i class="fa fa-trash-o fa-2x" aria-hidden="true" style="color:#007700"></i></a> </td>
															</tr> 
									<?php $no++;
									} ?>
									</tbody>  
