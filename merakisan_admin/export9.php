...
$cityid= $_SESSION['cityid']; 
	...
	//$sql = "SELECT p.name As `Name`, p.description As `Description`, p.pic_url As `Image Link`, pc.categoryname As `Category`,s.subcategory_name As `Subcategory`,p.product_id, p.cdate As `Date` ,cp.price As `Price`,cp.price_unit As `Unit`,c.city_name As `City`, cp.unit_value As `Unit Value` FROM `offer_productlist` p JOIN `product_subadmin`cp ON p.`product_id` = cp.`product_id`  JOIN  `city` c  ON  cp.city_id = c.id LEFT JOIN `product_category` pc  ON p.category_id = pc.category_id  LEFT JOIN `product_subcategory` s ON p.subcategory_id=s.id";
								...
								if(empty($cityid))
								{
									$sql = "SELECT d.`name`,d.`mobile`,o.`order_id`,o.`shipping_address`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status` FROM `assign_boy` a JOIN `delivery_boy` d ON a.`boyid`= d.`id` JOIN `orderdetail` o ON a.`order_id` = o.`order_id` WHERE o.`order_status`=2 GROUP BY o.`order_id` DESC";
								}
								else
								{
								$sql = "SELECT d.`name`,d.`mobile`,o.`order_id`,o.`shipping_address`,o.`payment_type`,o.`delivari_date`,o.`delivari_time`,o.`order_status` FROM `assign_boy` a JOIN `delivery_boy` d ON a.`boyid`= d.`id` JOIN `orderdetail` o ON a.`order_id` = o.`order_id` WHERE o.`city_id` = '$cityid' AND o.`order_status`=2 GROUP BY o.`order_id` DESC";
								}
